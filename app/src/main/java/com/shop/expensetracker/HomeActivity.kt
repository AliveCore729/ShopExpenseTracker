...
class HomeActivity : AppCompatActivity() {
    ...
    // Removed the hardcoded ADMIN_EMAILS list from here.
    ...
    // 🔹 Logic to Determine Role (UPDATED FOR DATABASE ADMIN CHECK)
    private fun checkUserRoleAndSetupUI() {
        val email = auth.currentUser?.email ?: ""

        if (email.isEmpty()) return

        // 1. Check if user is an Admin in the database
        db.collection("admins").document(email).get()
            .addOnSuccessListener { adminDoc ->
                if (adminDoc.exists()) {
                    ...
                    // ❌ NOT AN ADMIN: Check if user is marked as 'Staff'
                    isAdmin = false
            ...
            .addOnFailureListener {
                // Fallback to normal view if admin check fails (e.g., network error)
                isAdmin = false
                setupNormalUserView()
                loadShopBalance()
                loadUsers()
